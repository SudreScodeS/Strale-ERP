...
function enrichQuoteItems(items: QuoteItem[]): QuoteItem[] {
  ...
  return items.map(item => ({
    ...
    selectedVariables: item.selectedVariables.map(sv => {
      ...
      return { ...sv, groupName: sv.groupName || group?.name || sv.groupId, variableName: sv.variableName || variable?.name || sv.variableId };
    }),
  }));
}
...
export async function GET(request: Request) {
  try {
    ...
    if (quoteId) {
      ...
      if (payload.role === 'seller' && quote.userId !== payload.userId) return forbidden('Acesso negado.');
    ...
    if (payload.role === 'seller') quotes = quotes.filter(q => q.userId === payload.userId);
    ...
    return ok({ quotes: quotesWithCreator });
  } catch (error) {
...
export async function POST(request: Request) {
  try {
    ...
    const { customerName, name, items, logoColors, validDays, notes, deliveryDate } = body as {
      customerName?: string; name?: string; items: QuoteItem[]; logoColors?: number; validDays?: number; notes?: string; deliveryDate?: string;
    };

    if (!items || items.length === 0) return badRequest('Adicione pelo menos um item ao orçamento.');

    const quote = criarOrcamento(payload.userId, customerName || 'Cliente', name || `Orçamento ${new Date().toLocaleDateString('pt-BR')}`, items, logoColors || 0, validDays, notes, deliveryDate);
...
export async function PATCH(request: Request) {
  try {
    ...
    const { quoteId, action, status, deliveryDate, name } = body as {
      quoteId: string; action?: 'clone' | 'convert' | 'update-status'; status?: Quote['status']; deliveryDate?: string; name?: string;
    };

    if (!quoteId) return badRequest('ID do orçamento é obrigatório.');
    const quote = quoteData.getById(quoteId);
    if (!quote) return notFound('Orçamento não encontrado.');
    if (payload.role === 'seller' && quote.userId !== payload.userId) return forbidden('Você não pode modificar orçamentos de outros usuários.');
    ...
    if (action === 'convert') {
      if (!deliveryDate) return badRequest('A data de entrega é obrigatória para converter o orçamento em pedido.');
      ...
      logActivity(payload.userId, converter?.username || payload.userId, 'convert', 'quote', `Converteu orçamento "${quote.name}" em pedido #${result.order.id}`, quoteId);
...
export async function DELETE(request: Request) {
  try {
    ...
    const quote = quoteData.getById(quoteId);
    if (!quote) return notFound('Orçamento não encontrado.');
    if (payload.role === 'seller' && quote.userId !== payload.userId) return forbidden('Você não pode remover orçamentos de outros usuários.');
    if (quote.status !== 'draft' && quote.status !== 'rejected') return badRequest('Apenas orçamentos em rascunho ou rejeitados podem ser removidos.');
    ...
    return ok({ message: 'Orçamento removido com sucesso.' });
  } catch (error) {
    return fromError(error);
  }
}
