...
export async function POST(request: Request) {
  ...
  if (!isBootstrapRegistration) {
    try {
      ...
      return NextResponse.json(
        { message: 'Cadastro publico desativado. Crie usuarios pelo painel Usuarios com uma conta admin.' },
  ...
  if (!username || !email || !password) {
    return NextResponse.json({ message: 'Dados de registro incompletos.' }, { status: 400 });
  }
