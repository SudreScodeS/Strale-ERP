...
// Gera imagem de produto (sacola/camiseta/caneca) usando IA — Hugging Face FLUX.1-schnell
// A imagem gerada é usada como base; logo e cor são compostos em tempo real no client

import { NextResponse } from 'next/server';
import { requireRole } from '../../lib/auth';

// Cache em memória (por cor+estilo) — sobrevive durante a vida do server
...
/**
 * Gera prompt descritivo para o produto baseado na cor e estilo
 */
function buildPrompt(colorName: string, style: string): string {
  const stylePrompts: Record<string, string> = {
    sacola: `professional product photography of a clean empty tote bag, flat woven fabric shopping bag, rectangular shape, sturdy handles, clean front face, no text no logos no writing, solid ${colorName} color, studio lighting, white background, centered, high quality, commercial product photo, 4k, sharp focus, professional lighting`,
    camiseta: `professional product photography of a plain t-shirt, no text no logos no print, solid ${colorName} color, neatly folded, studio lighting, white background, centered, high quality, commercial product photo, 4k, sharp focus`,
...
export async function POST(request: Request) {
  try {
    ...
    // FLUX.1-schnell — rápido (~1-2s), qualidade decente
